...
// 주소 하나 가져오기
export async function fetchArticle(url, timeoutMs = 8000, externalSignal = null) {
  ...
  try {
    const publicUrl = parsePublicHttpUrl(url);
    if (!publicUrl) throw new Error('공개 웹 주소만 수집할 수 있습니다.');
    const res = await fetchPublicHttp(publicUrl.href, {
      credentials: 'omit',

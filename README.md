...
2. **피싱·광고 위험**: 본문을 근거로 `피싱 의심`, `광고성`, `로그인 필요`를 판별합니다. 위험 판정을 받은 글에 들어가면 경고 배너와 의심 문장을 표시합니다.
3. **출처 표시**: 검색 대상이 직접 운영하는 공식 사이트와 신뢰할 만한 공신력 출처를 구분해 검색 결과에 표시합니다.
...
| **피싱·광고 위험** | 피싱·광고·로그인 장벽을 탐지하고 위험 딱지와 근거를 표시 | 위험 판정을 받은 글에 경고 배너를 표시하고 의심 문장을 강조 |
...
이 프로젝트는 제목·본문 비교, 피싱·광고 위험 분석, 출처 분류에 Upstage Solar Pro 3를 사용했습니다. 개발 과정에서는 AI 코딩 도구를 활용했으며, 생성된 결과는 실제 Chrome 실행 화면과 원문을 기준으로 검증했습니다.

### 런타임 모델과 코드 위치

| 기능 | API 호출 | 프롬프트·검증 |
|---|---|---|
| 제목·본문 비교 | [`features/title/background.js`](unified-extension/features/title/background.js) | API 호출 파일과 동일한 파일에 프롬프트 및 응답 검증 로직 포함 |
| 피싱·광고 위험 | [`features/risk/lib/solar.js`](unified-extension/features/risk/lib/solar.js) | [`prompts.js`](unified-extension/features/risk/lib/prompts.js)에 프롬프트, [`verify.js`](unified-extension/features/risk/lib/verify.js)에 검증 로직 포함 |
| 출처 표시 | [`features/official/background.js`](unified-extension/features/official/background.js) | API 호출 파일과 동일한 파일에 프롬프트 및 응답 검증 로직 포함 |
...
|---|---|---|
| 구조 분석 | 세 기능의 코드 구조, 중복 로직 및 충돌 가능성 분석 | 통합본의 파일 구조와 기능별 책임 범위 결정 |
| 구현·리팩터링 | Manifest V3 통합, 자동 실행, 병렬 처리, 상태 복원, JSON 파서와 재시도 코드 작성 보조 | 실제 검색·기사 화면에서 자동 실행, 독립 토글, UI 배치와 하이라이트 확인 |
| 코드 검토 | 기능별 코드 검토 및 오류 가능성 분석 | 코드와 검토 결과를 대조하고 실제 구현과 맞는 수정만 반영 |
...
게시 전 `git diff`로 변경 범위를 확인하고 JavaScript 구문과 Manifest JSON 형식을 검사했습니다.
...
- 기사 본문에 포함된 지시문을 AI 명령으로 처리하지 않도록 프롬프트에 명확한 입력 데이터 경계를 설정했습니다.

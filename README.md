...
3. **공식·공신력 출처**: 검색 대상이 직접 운영하는 공식 사이트와 신뢰할 만한 공신력 출처를 구분해 검색 결과에 표시합니다.
...
별도 빌드, 패키지 설치, FastAPI·Node 서버 실행이 필요하지 않습니다.
...
6. 확장 목록에 **구라제거기**가 나타나는지 확인합니다.
7. 편하게 사용하려면 Chrome 툴바의 퍼즐 아이콘을 누르고 구라제거기를 고정합니다.

통합 실행본은 반드시 `unified-extension/`을 선택해야 합니다. 저장소 루트나 개별 기능 폴더를 선택하면 통합 UI가 실행되지 않습니다.
...
| **공식·공신력 출처** | 공식 사이트와 공신력 출처를 점으로 표시하고 호버 시 근거 제공 | 기사 내부에는 별도 표시하지 않음 |
...
- 점에 마우스를 올리거나 포커스하면 AI의 확신도와 판단 근거를 확인할 수 있습니다. 설명 팝오버는 최상위 레이어를 사용해 주소·메뉴와 공간상 겹치더라도 그 뒤에 가려지지 않습니다.
...
- 제목, 위험, 공식 출처 기능은 서로 기다리지 않고 병렬로 실행합니다. 한 기능이나 한 결과가 실패해도 나머지 분석은 계속됩니다.
...
└── features/
    ...
    └── official/                  # 공식성 분류, 2차 검증, 출처 점과 Popover
```
...
| 권한 | 사용 목적 |
|---|---|
| `storage` | API 키·기능 토글과 현재 실행의 짧은 진행 메타데이터 보관 |
| `tabs` | 검색 결과 링크의 실제 렌더링 본문을 비활성 탭에서 수집하고 수집 후 탭 닫기 |
| `scripting` | 확장 업데이트 직후 기존 탭에 Content Script가 없을 때 필요한 스크립트와 스타일 재주입 |
| `http://*/*`, `https://*/*` | 사용자가 분석한 공개 검색 결과 링크의 본문 접근과 기사 화면 강조 |

방문 기록, 다운로드, 클립보드 권한은 요청하지 않습니다. 네트워크로 가져올 URL은 별도의 공개 URL 검사도 통과해야 합니다.
...
- 확장 내부 구성 요소가 설정 상태를 확인할 수 있지만, 키를 분석 대상 사이트의 요청이나 DOM에는 전달하지 않습니다. 실제 외부 사용처는 Service Worker가 보내는 Solar API Authorization 헤더뿐입니다.
- 분석 진행률, 현재 검색·기사 연결 정보, 검증된 판정·하이라이트 메타데이터는 `chrome.storage.session`에 임시 보관합니다.
- 수집한 기사 전체 본문과 Solar 원문 응답을 별도 서버나 영구 JSON 파일로 축적하지 않습니다.
- 대회 안내에 따라 Upstage API 키는 주최 측이 보유하므로 별도 테스트 키 제출이 필요하지 않습니다.

## 심사 재현 시나리오

1. 위 설치 절차대로 `unified-extension/`을 로드하고 Upstage API 키를 저장합니다.
2. 세 기능을 모두 켭니다.
3. Google에서 `코스피 전망`처럼 일반 기사 결과가 여러 개 나오는 검색어를 입력합니다.
4. 팝업에서 기능별 진행률이 변하고 검색 결과 아래에 제목·본문 판정이 나타나는지 확인합니다. 위험 딱지는 위험 요소가 탐지된 결과에만 나타나므로, 정상 결과에 딱지가 없는 것은 정상 동작입니다.
5. 공식·공신력 출처 점에 마우스를 올려 확신도와 판단 근거를 확인합니다.
6. 분석된 일반 기사 링크를 열어 검색어 관련 문장 또는 위험 근거 문장이 본문에서 강조되는지 확인합니다.
7. Google로 돌아와 다음 검색 페이지로 이동하고 새 페이지 결과가 별도로 자동 분석되는지 확인합니다.
8. 기능 토글을 하나씩 꺼서 해당 기능의 UI만 사라지고 다른 두 기능은 계속 동작하는지 확인합니다.
...
- 위험 딱지와 공식 출처 점은 AI 보조 판정이며 법률·금융·보안상의 최종 판단을 대신하지 않습니다.

## 실행 문제 해결

- 코드를 갱신한 경우 `chrome://extensions`에서 구라제거기의 **새로고침**을 누른 뒤 기존 Google 검색 탭도 새로고침합니다.

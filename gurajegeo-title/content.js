...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message?.target === "google-content" && message.type === "START_ANALYSIS") {
    runCurrentPageAnalysis()
  ...
  if (message?.target === "google-content" && message.type === "RESULT_UPDATED") {
    if (isGoogleSearchPage() && isSameSearchPage(message.payload?.searchPageUrl, location.href)) {
  ...
  if (message?.target === "page-content" && message.type === "EXTRACT_RENDERED_ARTICLE") {
...
if (isGoogleSearchPage()) {
  ...
  addEventListener("pageshow", () => restoreCurrentSearchResults().catch(() => undefined));
...
function extractGoogleSearchContext() {
  ...
  for (const heading of document.querySelectorAll("h3")) {
    ...
    resultElements.set(resultId, { anchor, container });
  }
...
function isAdvertisement(container) {
  ...
  return /^(광고|스폰서|Sponsored)(?:\s|$)/i.test(normalizeText(container.textContent || ""));
}
...
function extractSnippet(container, title, documentType) {
  ...
  return normalizeText(container.textContent || "").replace(title, "").trim().slice(0, 1000);
}
...
function renderAnalysisComment(result) {
  ...
  const wrapper = ensureCommentWrapper(result.resultId);
  if (!wrapper) {
    return;
  }
...
function ensureCommentWrapper(resultId) {
  const elements = resultElements.get(resultId);
  if (!elements) {
    return null;
  }
...
function placeCommentWrapper(wrapper, elements) {
  ...
  const isVerticallyFlipped = hasVerticalAxisFlip(parent);
...
function hasVerticalAxisFlip(element) {
  if (!element) {
    return false;
  }
  ...
  try {
    ...
    // 매우 오래된 환경에서 DOMMatrixReadOnly를 사용할 수 없을 때의 최소 대안입니다.
...
function applyCommentOrientation(
  ...
  if (isVerticallyFlipped) {
    wrapper.style.setProperty("transform", "scaleY(-1)", "important");
  } else {
...
async function restoreCurrentSearchResults() {
  ...
  const response = await sendRuntimeMessage({ target: "background", type: "GET_CURRENT_RUN" });
  if (!response?.ok || !isSameSearchPage(response.data?.searchPageUrl, location.href)) {
    return;
  }
  ensureGoogleResultMap();
  for (const result of response.data.results || []) {
...
function getSearchIdentity(value) {
  try {
    ...
    return `${normalizeText(url.searchParams.get("q") || "")}::${Number(url.searchParams.get("start") || 0)}`;
  } catch {
    return null;
  }
}
...
function installArticleLifecycle() {
  ...
  addEventListener("pageshow", scheduleArticleRun);
  addEventListener("popstate", scheduleArticleRun);
  addEventListener("hashchange", scheduleArticleRun);
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "visible") {
      scheduleArticleRun();
    }
  ...
  setInterval(() => {
    if (location.href !== lastArticleUrl) {

...
// 영구 저장하지 않고 현재 확장 실행에 필요한 최소 상태만 메모리에 둡니다.
// Chrome이 Service Worker를 종료하거나 확장을 새로고침하면 자연스럽게 초기화됩니다.
...
chrome.action.onClicked.addListener(async (tab) => {
  const tabId = tab.id;
  if (!tabId || !isGoogleSearchUrl(tab.url || "")) {
    await showTemporaryBadge(tabId, "!", "#b3261e");
    console.warn("[구라제거기] 구글 검색 결과 페이지에서 실행해 주세요.");
    return;
  }

  try {
    const response = await startAnalysisInTab(tabId);
    if (!response?.ok) {
      throw new Error(response?.error || "검색 결과 분석에 실패했습니다.");
    }
    await showTemporaryBadge(tabId, "✓", "#137333");
  } catch (error) {
    console.error("[구라제거기] 분석 시작 실패", error);
    await showTemporaryBadge(tabId, "!", "#b3261e");
  }
});
...
async function startAnalysisInTab(tabId) {
  const message = { target: "google-content", type: "START_ANALYSIS" };
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "ANALYZE_PAGE") {
    operation = analyzeSearchPage(message.payload, sender.tab?.id);
  } else if (message.type === "MATCH_OPEN_ARTICLE") {
    operation = matchOpenArticle(message.payload, sender.tab?.id);
  } else if (message.type === "ANALYZE_OPEN_ARTICLE") {
    operation = analyzeOpenArticle(message.payload);
  } else if (message.type === "GET_CURRENT_RUN") {
    operation = Promise.resolve(currentRunState);
  } else {
    return false;
  }
...
async function collectSearchResultDocument(runId, searchResult) {
  return runWithCollectionSlot(async () => {
    ...
    const collectorTabId = await createCollectorTab();
    try {
      return await collectRenderedArticle(collectorTabId, searchResult);
    } finally {
...
async function createCollectorTab() {
 ...
 */
async function collectRenderedArticle(tabId, searchResult) {
  ...
  try {
    await navigateCollectorTab(tabId, normalizedUrl);
  } catch (error) {
  ...
  for (let attempt = 0; attempt < MAX_COLLECTION_ATTEMPTS; attempt += 1) {
    if (attempt > 0) {
      await reloadCollectorTab(tabId);
    }
    try {
      return await collectRenderedArticleAttempt(tabId, searchResult, normalizedUrl);
    } catch (error) {
      lastFailure = normalizeCollectionFailure(error);
      if (lastFailure.code === "body_missing") {
        const frameArticle = await collectArticleFromFrames(tabId, searchResult);
        if (frameArticle) {
          return frameArticle;
        }
      }
...
async function collectRenderedArticleAttempt(tabId, searchResult, normalizedUrl) {
  let lastError;
...
async function reloadCollectorTab(tabId) {
  try {
 ...
 */
async function collectArticleFromFrames(tabId, searchResult) {
  let injections;
  try {
  ...
  for (let attempt = 0; attempt < SOLAR_MAX_ATTEMPTS; attempt += 1) {
    try {
      const extraInstruction = lastParseError
        ...
        : "";
      const response = await runWithSolarRequestSlot(() => fetch(UPSTAGE_CHAT_URL, {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${CONFIG.UPSTAGE_API_KEY.trim()}`,
      ...
      if (attempt < SOLAR_MAX_ATTEMPTS - 1 && error instanceof TypeError) {
...
function assertLocalConfig() {
  if (!CONFIG.UPSTAGE_API_KEY?.trim() || CONFIG.UPSTAGE_API_KEY.includes("발급받은")) {
    throw new Error("config.local.js에 UPSTAGE_API_KEY를 입력해 주세요.");
  }
